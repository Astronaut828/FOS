...
export const ContractData = () => {
  ...
  useEffect(() => {
    const fetchCidContents = async () => {
      ...
      setCidContents(contents.reverse());
    };
      ...
      >
        ...
        <div className="flex justify-between w-full px-20 items-center">
          ...
          <div className="flex gap-4">
            <button className="btn" onClick={showPreviousPost} disabled={currentPostIndex === 0}>
              Previous
            </button>
            <button className="btn" onClick={showNextPost} disabled={currentPostIndex === cidContents.length - 1}>
              Next
            </button>
          </div>
        ...
        <div className="flex flex-col justify-center items-center rounded-3xl bg-base-300 py-10 px-13 mt-5 lg:py-auto w-full max-w-[95vw]">
          ...
          {address && cidContents.length > 0 ? (
            <div className="mt-2 p-4 px-10">
              {/* CID and Timestamp with emphasized labels */}
              <p className="text-m mb-2">
                <strong>IPFS CID:</strong> {currentPost.cid} -<strong> Timestamp:</strong>{" "}
                {currentPost.content.timestamp
                  ? new Date(currentPost.content.timestamp).toLocaleString()
                  : "Unavailable"}
              </p>
